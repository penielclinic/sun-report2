...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: report } = await admin.from("mission_reports").select("id, mission_id, status, report_date").eq("id", id).maybeSingle();
  ...
  if (session.profile.role === "mission_leader") {
    ...
    if (!isOpenReportWeek(report.report_date)) return jsonError("보고 기간이 끝난 보고서는 지울 수 없어요. 담임목사님께 요청해 주세요", 409);
  }
